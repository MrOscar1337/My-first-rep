...
class subjectIndex {          // Структура для хранения информации о поезде
public:
    string word;
    int *pages;
    int count_page = 0;
public:
    subjectIndex(string wrd, int pg) {            // Конструктор класс с одним словом и одной страницей
        word = wrd;
        if (count_page>9) {
            cout << "Предметный указатель не может хранить больше 10 номеров сnраниц" << endl;
        }
        pages[count_page] = pg;
        count_page++;
    }
    subjectIndex(string wrd, int *pgs) {                // Конструктор класса по умолчанию
        word = wrd;
        if (*(&pgs + 1) - pgs > 10) {
            cout << "Предметный указатель не может хранить больше 10 номеров сnраниц" << endl;
        }
        count_page = *(&pgs + 1) - pgs;
        pages = pgs;
    }
    subjectIndex() {                // Конструктор класса по умолчанию

    }
    void printSubjectIndex() {     // Функция отображения структуры полностью
        cout << "Слово " << word << " встречается на следующих страницах" << endl;
...
public:
    Node(subjectIndex tra) {           // Конструктор структуры с передачей в неё параметров
        index = tra.word;
        tr = tra;
        next = nullptr;
    }
...
bool exists(Node* head, subjectIndex ind);
bool exists(Node* head, int index, int day, int month, int year);
void insert(Node*& head, subjectIndex tr);
...
int main()
{
    ...
    while (work) {          // Бесконечный цикл опроса
        ...
        case 1:             // Ввод данных
        {
            ...
            cin >> fio;
            subjectIndex tr(number, end_ptr, fio, day, month, year);       // Создание структуры поезда по конструктору с параметрами
...
bool exists(Node* head, subjectIndex ind)        // Функция проверки, существует ли уже поезд вовсе
{
...
bool exists(Node* head, int number, int day, int month, int year)         // Перегрузка функции проверки существует ли поезд с определённым номером
{
    ...
    if (head->index == number && head->tr.day == day && head->tr.month == month && head->tr.year == year) { // Если да
        head->tr.printSubjectIndex();  // Отобразить информацию о поезде
...
#pragma region Удаление элемента списка
...
void insert(Node*& head, subjectIndex tr)    // Фнкция добавления узлов, принемает двойной указатель на корень дерева и структуру поезда, которую надо хранить
{
